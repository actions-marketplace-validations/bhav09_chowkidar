...
class SLMClient:
    ...
    def classify_purposes(self, project: str, references: list) -> dict[tuple[str, str], str] | None:
        """Classify model usage purposes via local SLM. Returns None on failure."""
        if not self.config.get("slm_enabled", False):
            return None

        ollama = _get_ollama()
        if ollama is None or not self.is_available():
            return None

        from ..cloud_connectors import build_classification_payload, normalize_use_case
        from .prompts import format_purpose_classification_prompt, parse_purpose_classification_response

        payload = build_classification_payload(project, references)
        prompt = format_purpose_classification_prompt(payload)
        ...
        try:
            ...
            raw_response = response.get("response", "") if isinstance(response, dict) else response.response
            parsed = parse_purpose_classification_response(raw_response)
            if not parsed:
                return None
            return {
                (item["variable"], item["file"]): normalize_use_case(item["use_case"])
                for item in parsed
            }
        except Exception as e:
            logger.warning("SLM purpose classification failed: %s", e)
            return None
